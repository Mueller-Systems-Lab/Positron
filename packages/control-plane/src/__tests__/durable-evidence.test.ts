...
describe('DURABLE_DECISION_RECONCILIATION', () => {
	it('preserves the historical decision and resolves the additive reconciliation', () => {
		const original = storeDecision(db, 'run-1', 'BLOCKED', 'NO_VERIFICATION', '{}', '2026-08-29T00:00:00.000Z');
		const result = reconcileDecision(db, {
			runId: 'run-1', sourceDecisionId: original.decision_id, jobId: 'job-1', attemptId: 'attempt-1',
			previousDecision: 'BLOCKED', reconciledDecision: 'DONE', reasonCode: 'AUTHORITATIVE_TERMINAL_RECONCILIATION',
			evidenceRefs: ['run:run-1', 'github:pr:1'], evidenceHashes: [HASH, HASH],
			originalEventTime: original.created_at, reconciliationTime: '2026-08-30T00:00:00.000Z',
		});
	...
	it('makes an identical reconciliation idempotent and rejects conflicts', () => {
		...
		const input = {
			runId: 'run-1', sourceDecisionId: original.decision_id, jobId: 'job-1', attemptId: 'attempt-1',
			previousDecision: 'BLOCKED', reconciledDecision: 'DONE', reasonCode: 'RECONCILED',
			evidenceRefs: ['evidence:1'], evidenceHashes: [HASH], reconciliationTime: '2026-08-30T00:00:00.000Z',
		};
		...
		expect(() => reconcileDecision(db, { ...input, reconciledDecision: 'FAILED' })).toThrow('DECISION_RECONCILIATION_CONFLICT');
	});
	...
	it('does not resolve or expose reconciliations from another run', () => {
		...
		reconcileDecision(db, {
			runId: 'run-1', sourceDecisionId: one.decision_id, jobId: 'job-1', attemptId: 'attempt-1',
			previousDecision: 'BLOCKED', reconciledDecision: 'DONE', reasonCode: 'RECONCILED',
			evidenceRefs: ['evidence:1'], evidenceHashes: [HASH],
		});
...
function approvalInput(overrides: Record<string, unknown> = {}) {
	return {
		approvalFingerprint: HASH, runId: 'run-1', queueItemId: 'queue-1', jobId: 'job-1', attemptId: 'attempt-1',
		repository: 'Mueller-Systems-Lab/positron-308-sandbox', repositoryId: '1349145121', baseSha: HASH,
		effectManifestHash: HASH, branchIdentity: 'positron/issue-308-stage3-pilot', filePath: 'stage3/positron-supervised-pilot.md',
		fileSha256: HASH, commitMetadataSha256: HASH, prMetadataSha256: HASH, approvalExpiresAt: '2026-08-30T02:00:00.000Z',
		consumedAt: '2026-08-30T00:00:00.000Z', idempotencyKey: 'run-1:job-1:attempt-1', approvalSchemaVersion: 'stage3-run-bound-approval-v1',
		attemptLeaseGeneration: 1, workspaceLockGeneration: 2, ...overrides,
	};
}
...
describe('DURABLE_APPROVAL_CONSUMPTION', () => {
	it('stores the exact non-secret authority binding before a writer can use it', () => {
		...
		const columns = (db.prepare('PRAGMA table_info(cp_approval_consumptions)').all() as Array<{ name: string }>).map((row) => row.name);
		expect(columns.some((column) => /^(token|pat|authorization|authorization_header)$/i.test(column))).toBe(false);
	});

	it('rejects approval replay and preserves idempotency binding', () => {
		persistApprovalConsumption(db, approvalInput());
		expect(() => persistApprovalConsumption(db, approvalInput())).toThrow('APPROVAL_CONSUMPTION_REPLAY');
		expect(() => persistApprovalConsumption(db, approvalInput({ approvalFingerprint: 'b'.repeat(64) }))).toThrow('APPROVAL_CONSUMPTION_REPLAY');
	});

	it('marks retrospective evidence honestly and requires source evidence', () => {
		expect(() => reconstructApprovalConsumption(db, approvalInput())).toThrow('RECONSTRUCTED_APPROVAL_SOURCE_MISSING');
		const record = reconstructApprovalConsumption(db, approvalInput({
			sourceEvidenceRefs: ['execution:stage3-audit.jsonl'], sourceEvidenceHashes: [HASH],
		}));
	...
	it('rejects secret-like material instead of persisting it', () => {
		expect(() => persistApprovalConsumption(db, approvalInput({ repository: 'Bearer should-never-be-stored' }))).toThrow('APPROVAL_SECRET_INPUT_REJECTED');
	});
});

...
describe('RUNTIME_SOAK (disposable workspaces, real runs)', () => {
	...
	it('SOAK_RUN_A — Happy Path: roter Test → DONE mit grünem Test', async () => {
		...
		const result = await runDurableRun(
			{
				db,
				workspace: { path: wsA.dir, repositoryRef: 'xxammaxx/soak-workspace', readHead: wsA.readHead },
				buildWorker: worker,
				verifyTool,
		...
		const buildAttempts = listJobAttempts(db, result.jobs.find((j) => j.job_type === 'build')!.job_id);
	...
	it('SOAK_RUN_B — Fix Path: VERIFY FAIL → new_evidence → strategy_delta → DONE', async () => {
		...
		const result = await runDurableRun(
			{
				db,
				workspace: { path: wsB.dir, repositoryRef: 'xxammaxx/soak-workspace', readHead: wsB.readHead },
				buildWorker: worker,
				verifyTool,
				reviewFindings: async () => [],
				maxAttempts: 3,
	...
	it('SOAK_RUN_C — Blind Retry Denial: identischer Versuch → RETRY_DENIED_NO_STRATEGY_DELTA, kein 2. Worker-Call', async () => {
		...
		const silentVerifyTool = {
			async run(ctx: { run_id: string; job_id: string; attempt_id: string; workspacePath: string }) {
		...
		const result = await runDurableRun(
			{
				db,
				workspace: { path: wsC.dir, repositoryRef: 'xxammaxx/soak-workspace', readHead: wsC.readHead },
				buildWorker: worker,
	...
	it('SOAK_RUN_D — Security Block: grüne Gates + CRITICAL-Finding → BLOCKED (SECURITY_BLOCK)', async () => {
		...
		const result = await runDurableRun(
			{
				db,
				workspace: { path: wsD.dir, repositoryRef: 'xxammaxx/soak-workspace', readHead: wsD.readHead },
				buildWorker: worker,
				verifyTool,
	...
	it('SOAK_RUN_E — Recovery: Crash nach verify → Resume ohne Rerun, keine duplicate mutation', async () => {
		...
		const crashRun = await runDurableRun(
			{
				db,
				workspace: { path: wsE.dir, repositoryRef: 'xxammaxx/soak-workspace', readHead: wsE.readHead },
				buildWorker: worker,
				verifyTool,
				reviewFindings: async () => [],
	...
	it('SOAK_RUN_F — Parallelism Negative Canary: sequentielles Research → PARALLELISM_NOT_PROVEN', async () => {
		...
		const result = await runDurableRun(
			{
				db,
				workspace: { path: wsF.dir, repositoryRef: 'xxammaxx/soak-workspace', readHead: wsF.readHead },
				buildWorker: worker,
				verifyTool,
				reviewFindings: async () => [],
	...
	it('SOAK_TRACE_COMPLETENESS — wesentliche Jobs sind lückenlos nachvollziehbar', () => {
		const runs = ['soak_a_happy', 'soak_b_fix', 'soak_c_denial', 'soak_d_security', 'soak_e_recovery', 'soak_f_negative'];
	...
	it('SOAK_CLEANUP — Workspaces werden entfernt (disposable)', () => {

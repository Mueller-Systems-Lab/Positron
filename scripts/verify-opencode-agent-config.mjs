...
assert(config.agent?.["issue-orchestrator"]?.mode === "primary", "issue-orchestrator must be primary");
...
assert(
  JSON.stringify(Object.keys(taskRules).filter((key) => taskRules[key] === "allow").sort()) === JSON.stringify([...required].sort()),
...
for (const command of dangerous) assert(controller.permission?.bash?.[command] === "deny", `controller deny missing: ${command}`);
...
for (const id of ["build", "plan", "general", "explore"]) assert(new RegExp(`^${id} \\(`, "m").test(listed.stdout), `built-in agent not discovered: ${id}`);
...
for (const id of ["issue-orchestrator", ...required]) {
  ...
  assert(resolved.mode === (id === "issue-orchestrator" ? "primary" : "subagent"), `${id} mode is not resolved correctly`);
  const rules = resolved.permission ?? [];
  const resolve = (permission, resource = "*") => rules.filter((rule) => rule.permission === permission && (rule.pattern === "*" || rule.pattern === resource)).at(-1)?.action;
  if (id !== "issue-orchestrator") {
    assert(resolve("edit") === "deny" && resolve("write") === "deny", `${id} write boundary is not effective`);
    ...
    assert(resolve("bash", "git push origin forbidden") === "deny", `${id} shell boundary is not effective`);
  }
}

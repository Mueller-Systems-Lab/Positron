...
export function extractTelemetry(events, meta, verificationPassed, elapsedMs) {
	...
	const firstPatch = toolEvents.find((event) =>
		['edit', 'write', 'apply_patch'].includes(event.part?.tool),
	);
	...
	const tokenTotals = events
		...
		.map((event) => event.part?.tokens?.total)
	...
	return {
		...
		time_to_first_patch_ms:
			firstPatch && firstEvent ? firstPatch.timestamp - firstEvent.timestamp : null,
...
export function aggregate(rows) {
	...
	const numeric = (key) =>
		successful.map((row) => row[key]).filter((value) => typeof value === 'number');
	return {
		...
		repeated_read_rate: valid.length
			? valid.reduce((sum, row) => sum + row.repeated_reads, 0) / valid.length
			: null,
		token_provenance:
			successful.length &&
			successful.every((row) => row.token_provenance === 'VERIFIED_PROVIDER_REPORTED')
				? 'VERIFIED_PROVIDER_REPORTED'
				: 'UNKNOWN',
		cost_per_verified_success: 'NOT_AVAILABLE',
		failure_classes: Object.fromEntries(
			[
				...new Set(
					rows
						.filter((row) => !row.valid_runtime_attempt || !row.verified_success)
						.map((row) => row.failure_class)
						.filter(Boolean),
				),
			].map((value) => [value, rows.filter((row) => row.failure_class === value).length]),
		),
	};
}
...
export function evaluateValueGate(arms) {
	...
	if (
		[a, b, c].some(
			(arm) => arm.sample_size < MIN_SAMPLE_SIZE || arm.valid_runtime_attempts < MIN_SAMPLE_SIZE,
		)
	) {
		return {
	...
	const qualityNonInferior =
		b.verified_success_rate >=
		Math.min(a.verified_success_rate, c.verified_success_rate) - NON_INFERIORITY_MARGIN;
	...
	if (!qualityNonInferior)
		return {
			classification: 'GREEN_POSITRON_EXPLORATION_OPTIMIZATION_REJECTED_QUALITY_REGRESSION',
			reason_code: 'VERIFIED_SUCCESS_REGRESSION',
			quality_non_inferior: false,
			efficiency_improved: efficiencyImproved,
			compute_explains_gain: false,
			compute_matched: computeMatched,
		};
	if (!efficiencyImproved)
		return {
			classification: 'GREEN_POSITRON_EXPLORATION_OPTIMIZATION_REJECTED_NO_MARGINAL_VALUE',
			reason_code: 'EXPLORATION_NO_MARGINAL_UTILITY',
	...
	return {
		classification: 'GREEN_POSITRON_EXPLORATION_EFFICIENCY_VALUE_PROVEN',
		reason_code: 'EXPLORATION_EFFICIENCY_VALUE_PROVEN',
		quality_non_inferior: true,
...
export function negativeCanary() {
	const telemetry = {
		strategy_id: 'BROAD_REPETITIVE_V0',
		files_read: 24,
		unique_files_read: 6,
		repeated_reads: 18,
		context_admitted: CONTEXT_BUDGET * 2,
	};
	const rejected =
		telemetry.repeated_reads > telemetry.unique_files_read ||
		telemetry.context_admitted > CONTEXT_BUDGET;
	return {
		candidate_id: telemetry.strategy_id,
		rejected,
		reason_code: rejected ? 'EXCESSIVE_CONTEXT_OR_REPEATED_READS' : 'UNEXPECTED_ACCEPTANCE',
